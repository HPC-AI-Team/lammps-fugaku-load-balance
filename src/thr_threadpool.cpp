...
ThrThreadpool::ThrThreadpool(LAMMPS *ptr, int style)
  : lmp(ptr), fix(nullptr), thr_style(style), thr_error(0)
{
  // register fix omp with this class
  int ifix = lmp->modify->find_fix("threadpool");
  if (ifix < 0)
    lmp->error->all(FLERR,"The 'package threadpool' command is required for /threadpool styles");
  fix = static_cast<FixThreadpool *>(lmp->modify->fix[ifix]);
}
...
void ThrThreadpool::ev_setup_thr(int eflag, int vflag, int nall, double *eatom,
...
void ThrThreadpool::reduce_thr(void *style, const int eflag, const int vflag,
                        ThrData *const thr)
{
  ...
  case THR_PAIR: {
    ...
    // if(lmp->comm->me == 0) utils::logmesg(lmp, "evflag {}\n", evflag); 
      ...
      if(tid == 11) {
        ...
        for(int i = 0; i < nthreads; i++) {
          ...
          if (eflag & ENERGY_GLOBAL) {
            ...
            pair->eng_coul += td->eng_coul;

            // mesg += fmt::format(" {}:{}  ", td->eng_vdwl, td->eng_coul);
            td->eng_vdwl = 0.0;
            td->eng_coul = 0.0;
          }
          if (vflag & (VIRIAL_PAIR | VIRIAL_FDOTR)){
            for (int i=0; i < 6; ++i) {
              pair->virial[i] += td->virial_pair[i];
              td->virial_pair[i] = 0.0;
            }
          }
        }
        // mesg += "\n";
        // if(lmp->comm->debug_flag)  utils::logmesg(lmp, mesg);
      }

      lmp->parral_barrier(12, tid);

      // if(lmp->comm->debug_flag)  utils::logmesg(lmp,"[info] finish deduce vflag {} \n", tid);

      if (eflag & ENERGY_ATOM) {
        data_reduce_thr_threadpool(&(pair->eatom[0]), nall, nthreads, 1, tid, lmp);
      }
      // per-atom virial and per-atom centroid virial are the same for two-body
      // many-body pair styles not yet implemented
      if (vflag & (VIRIAL_ATOM | VIRIAL_CENTROID)) {
        data_reduce_thr_threadpool(&(pair->vatom[0][0]), nall, nthreads, 6, tid, lmp);
      }
      // check cvatom_pair, because can't access centroidstressflag
      if ((vflag & VIRIAL_CENTROID) && thr->cvatom_pair) {
        data_reduce_thr_threadpool(&(pair->cvatom[0][0]), nall, nthreads, 9, tid, lmp);
      }
    }
  }
    break;
  ...
  case THR_BOND:
    ...
    if (evflag) {
      ...
      if (eflag & ENERGY_ATOM) {
        data_reduce_thr_threadpool(&(bond->eatom[0]), nall, nthreads, 1, tid, lmp);
      }
      // per-atom virial and per-atom centroid virial are the same for bonds
      if (vflag & (VIRIAL_ATOM | VIRIAL_CENTROID)) {
        data_reduce_thr_threadpool(&(bond->vatom[0][0]), nall, nthreads, 6, tid, lmp);
      }
  ...
  case THR_ANGLE:
    ...
    if (evflag) {
      ...
      if (eflag & ENERGY_ATOM) {
        data_reduce_thr_threadpool(&(angle->eatom[0]), nall, nthreads, 1, tid, lmp);
      }
      if (vflag & VIRIAL_ATOM) {
        data_reduce_thr_threadpool(&(angle->vatom[0][0]), nall, nthreads, 6, tid, lmp);
      }
      if (vflag & VIRIAL_CENTROID) {
        data_reduce_thr_threadpool(&(angle->cvatom[0][0]), nall, nthreads, 9, tid, lmp);
      }
  ...
  case THR_DIHEDRAL:
    ...
    if (evflag) {
      ...
      if (eflag & ENERGY_ATOM) {
        data_reduce_thr_threadpool(&(dihedral->eatom[0]), nall, nthreads, 1, tid, lmp);
      }
      if (vflag & VIRIAL_ATOM) {
        data_reduce_thr_threadpool(&(dihedral->vatom[0][0]), nall, nthreads, 6, tid, lmp);
      }
      if (vflag & VIRIAL_CENTROID) {
        data_reduce_thr_threadpool(&(dihedral->cvatom[0][0]), nall, nthreads, 9, tid, lmp);
      }
  ...
  case THR_IMPROPER:
    ...
    if (evflag) {
      ...
      if (eflag & ENERGY_ATOM) {
        data_reduce_thr_threadpool(&(improper->eatom[0]), nall, nthreads, 1, tid, lmp);
      }
      if (vflag & VIRIAL_ATOM) {
        data_reduce_thr_threadpool(&(improper->vatom[0][0]), nall, nthreads, 6, tid, lmp);
      }
      if (vflag & VIRIAL_CENTROID) {
        data_reduce_thr_threadpool(&(improper->cvatom[0][0]), nall, nthreads, 9, tid, lmp);
      }
  ...
  if (style == fix->last_omp_style) {
    ...
    if (lmp->atom->torque)
      data_reduce_thr_threadpool(&(lmp->atom->torque[0][0]), nall, nthreads, 3, tid, lmp);
  }
...
void ThrThreadpool::e_tally_thr(Pair * const pair, const int i, const int j,
...
------------------------------------------------------------------------- */
void ThrThreadpool::v_tally_thr(Pair * const pair, const int i, const int j,
                         const int nlocal, const int newton_pair,
...
void ThrThreadpool::ev_tally_thr(Pair * const pair, const int i, const int j, const int nlocal,
...
void ThrThreadpool::ev_tally_xyz_thr(Pair * const pair, const int i, const int j,
                              const int nlocal, const int newton_pair,
...
void ThrThreadpool::ev_tally_xyz_full_thr(Pair * const pair, const int i,
                                   const double evdwl, const double ecoul,
...
void ThrThreadpool::ev_tally3_thr(Pair * const pair, const int i, const int j, const int k,
                           const double evdwl, const double ecoul,
...
void ThrThreadpool::ev_tally4_thr(Pair * const pair, const int i, const int j,
...
void ThrThreadpool::ev_tally_list_thr(Pair * const pair, const int key,
...
void ThrThreadpool::ev_tally_thr(Bond * const bond, const int i, const int j, const int nlocal,
...
void ThrThreadpool::ev_tally_thr(Angle * const angle, const int i, const int j, const int k,
...
void ThrThreadpool::ev_tally13_thr(Angle * const angle, const int i1, const int i3,
...
void ThrThreadpool::ev_tally_thr(Dihedral * const dihed, const int i1, const int i2,
...
void ThrThreadpool::ev_tally_thr(Improper * const imprp, const int i1, const int i2,
                          const int i3, const int i4, const int nlocal,
...
void ThrThreadpool::v_tally2_thr(Pair *const pair, const int i, const int j, const double fpair,
...
void ThrThreadpool::v_tally2_newton_thr(Pair *const pair, const int i, const double * const fi,
...
void ThrThreadpool::v_tally3_thr(Pair *const pair, const int i, const int j, const int k,
                          const double * const fi, const double * const fj,
...
void ThrThreadpool::v_tally4_thr(Pair *const pair, const int i, const int j, const int k, const int m,
                          const double * const fi, const double * const fj,
                          const double * const fk, const double * const drim,
                          const double * const drjm, const double * const drkm,
                          ThrData * const thr)
...
double ThrThreadpool::memory_usage_thr()
{
  double bytes=0.0;
