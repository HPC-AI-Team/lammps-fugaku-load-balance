...
void PPPM::compute(int eflag, int vflag)
{
  ...
  // self_timer->stamp();
  particle_map();
  // self_timer->stamp(Timer::PARTICLE_MAP);
  make_rho();
  // self_timer->stamp(Timer::MAKE_RHO);
  ...
  // self_timer->stamp(Timer::GC_REVERSE);
  
  brick2fft();
  // self_timer->stamp(Timer::BRICK2FFT);
  ...
  poisson();
  // self_timer->stamp(Timer::POISSON);
