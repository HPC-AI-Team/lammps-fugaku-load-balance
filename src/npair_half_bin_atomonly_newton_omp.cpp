...
void NPairHalfBinAtomonlyNewtonOmp::build_parral(NeighList *list, int tid)
{
  const int nlocal = (includegroup) ? atom->nfirst : atom->nlocal;
  ...
  // if(DEBUG_MSG) utils::logmesg(lmp, " build_parrale omp  tid {} ifrom {} ito {}  nlocal {} \n", tid, ifrom, ito, nlocal); 
  ...
  if(DEBUG_MSG) utils::logmesg(lmp, "[INFO] build_parral list->inum {} \n", nlocal );  
}
